...
class CRASS(Dataset):
    def __init__(self, dataset_type="validation"):
        self.data = pd.read_csv("promptbench/data/crass.csv")
...
class GLUE(Dataset):
    def __init__(self, task, sample_cnt, dataset_type="validation"):
        ...
        self.task_to_cnt = {'sst2': 872, 'cola': 1043, 'qqp': 40430, 'mrpc': 408, 'mnli': 19647, 'mnli_matched': 9815, 'mnli_mismatched': 9832, 'qnli': 5463, 'rte': 277, 'wnli': 71}
        if self.task == "mnli":
            from datasets import concatenate_datasets
            matched = load_dataset('glue', 'mnli', cache_dir="/mnt/dolphinfs/hdd_pool/docker/user/hadoop-mtai/users/yangxin106/datasets/")["validation_matched"]
            mismatched = load_dataset("glue", "mnli", cache_dir="/mnt/dolphinfs/hdd_pool/docker/user/hadoop-mtai/users/yangxin106/datasets/")["validation_mismatched"]
            self.data = concatenate_datasets([matched, mismatched])
        else:
            self.data = load_dataset("glue", task, cache_dir="/mnt/dolphinfs/hdd_pool/docker/user/hadoop-mtai/users/yangxin106/datasets/")[dataset_type]

        # Select at most sample_cnt
        # Note: remember to use the same data for evaluating continual instruction tuning model and CoIN
        random_indices = random.sample(range(len(self.data)), min(self.task_to_cnt[task], sample_cnt))
        self.data = self.data.select(random_indices)
    ...
    def get_content_by_idx(self, idx, task, *args):
        ...
        elif task == 'mnli' or task == 'mnli_matched' or task == 'mnli_mismatched' or 'anli' in task or task == 'snli':
            content = 'Premise: ' + \
        ...
        elif task == 'rte' or task == 'mrpc' or task == "wnli" or task == "paws":
            content = 'Sentence 1: ' + \
...
def create_dataset(dataset_name, *args):
    if dataset_name in ["cola", "sst2", "qqp", "mnli", "mnli_matched", "mnli_mismatched", "qnli", "wnli", "rte", "mrpc"]:
        return GLUE(dataset_name, args[0])
    elif dataset_name == 'mmlu':
        return MMLU()
    elif dataset_name == 'crass':
        return CRASS()
