...
@dataclass
class ContrastiveQwen2Output(ModelOutput):
...
class ContrastiveQwen2(Qwen2PreTrainedModel):
    def __init__(self, config, do_predict=False, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)

        # From original Qwen2ForCausalLM
        self.model = Qwen2Model(config)
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)

        # Parameters for Contrastive loss
        self.pooling_method = getattr(config, 'pooling_method', 'last')
        self.temperature = getattr(config, 'temperature', 0.2)
        self.contrastive_loss = ContrastiveLoss(self.temperature)
        self.contrastive_loss_ratio = getattr(config, 'contrastive_loss_ratio', 1.0)

        self.do_predict = do_predict
        self.do_contrastive = getattr(config, 'do_contrastive', True)
    ...
    def get_pooled_hidden_states(self, hidden_states):
        """
        Get hidden states of the last token of each sequence (reference: Qwen2ForSequenceClassification)
    ...
    def forward(
            ...
            return_dict: Optional[bool] = None,
            labels: Optional[torch.LongTensor] = None,
    ) -> Union[ContrastiveQwen2Output, ContrastiveEvalOutput]:
