...
class Inference(object):
    ...
    def create_model(self):
        ...
        if self.model not in ['chatgpt', 'gpt4']:
            ...
            elif self.model == "contrastive_qwen2":
                from transformers import Qwen2PreTrainedModel, Qwen2Tokenizer
    ...
    def predict(self, prompt=None, info=''):
        ...
        if self.model in ["chatgpt", "gpt4"]:
            ...
            results, result_df = self.predict_by_local_inference(self.model, prompt, info)
    ...
    def predict_by_local_inference(self, model, prompt, info=''):
        data_len = len(self.args.data)
        if data_len > 1000:
            data_len = 1000
        ...
        for idx in tqdm(range(data_len)):
            ...
            raw_pred = self.pred_by_generation(input_text, model, info+f'/{idx}')
    ...
    def pred_by_generation(self, input_text, model, info=""):
        out = 'error!'
        ...
        if 't5' in model or 'ul2' in model:
            ...
            peft_model_generate_kwargs = {"input_ids": input_ids, "temperature": 0,
                                          "max_new_tokens": self.args.generate_len,
                                          "pad_token_id": self.tokenizer.eos_token_id,"return_dict_in_generate": True, "output_hidden_states": True    }
            outputs = self.pipe.generate(**peft_model_generate_kwargs)
            last_token_last_layer_hidden = outputs.hidden_states[-1][-1][:, -1, :]
            # print(outputs.hidden_states.shape)
            # print(last_token_last_layer_hidden.shape)
            # print(last_token_last_layer_hidden)
            out = self.tokenizer.decode(outputs.sequences[0], skip_special_tokens=True)
        
        elif model == "contrastive_qwen2":
            peft_model_generate_kwargs = {"input_ids": input_ids, "temperature": 0.1,
                                          "max_new_tokens": self.args.generate_len,
                                          "pad_token_id": self.tokenizer.eos_token_id, "return_dict_in_generate": True, "output_hidden_states": True }
            outputs = self.pipe.generate(**peft_model_generate_kwargs)
            last_token_last_layer_hidden = outputs.hidden_states[-1][-1][:, -1, :]
